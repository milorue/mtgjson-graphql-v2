...
import { getDailyPriceFile, getPrices } from "../APIOperators";
...
const ingestPrices = async () => {
  ...
  try {
    // TODO: do it via api at some point
    // const allPrices = await getPrices()
    const allPrices = getDailyPriceFile();
    ...
    cardsWithUUID.map(async (cardEntity: CardEntity, index) => {
      try {
        ...
        priceFormat = allPrices[cardEntity.uuid];
